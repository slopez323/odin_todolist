import './style.css';
import {
  changeView, displayTasks, hideDetails, showDetails, addProjOption, showAllProjects,
} from './read';
...
import {
  showInputPopup, checkInputs, checkProjSelection, addProject,
} from './inputs';

(() => {
  taskFunctions.setTasks(JSON.parse(localStorage.getItem('taskList')) || []);
  projectFunctions.setProjects(JSON.parse(localStorage.getItem('projList')) || []);
...
(() => {
  ...
  document.querySelector('.nav-proj+ul').addEventListener('keypress', (e) => {
    if (e.target.tagName === 'INPUT') {
      if (e.key === 'Enter' && e.target.value !== '') createProject(e.target.value);
    }
  });
